      ...
      if (filter == todo.status || filter == "all") {
        ...
        liTag += `<li class="task">
            <label for="${id}">
              ...
              <p class="deadline task-info">Deadline: ${formatDate(
                todo.deadline
              )}</p>
        ...
        if (todo.subtasks && todo.subtasks.length > 0) {
          ...
          todo.subtasks.forEach((subtask, subtaskId) => {
            let subtaskCompleted = subtask.status === "completed" ? "checked" : "";
            liTag += `<li class="subtask">
                ...
                <div class="subtask-settings">
                  <ul class="task-menu">
                    <button class="inside_btn3" onclick='editSubtask(${id}, ${subtaskId}, "${
              subtask.name
...
function updateSubtaskStatus(taskId, subtaskId) {
  ...
  addToSubtaskActivityLog(
    todos[taskId].name,
    todos[taskId].subtasks[subtaskId].name,
...
function containsSubtask(subtasks, searchStr) {
  ...
  return subtasks.some(
    (subtask) => subtask.name.toLowerCase().indexOf(searchStr) > -1
  );
}
    ...
    filteredTodos.forEach((todo, id) => {
              ...
              <p class="tags task-info">Tags: ${tagsFormatted}</p>
                  ...
                  <div class="settings">
                      <ul class="task-menu">
                          ...
                          <button class="inside_btn" onclick='editTask(${id}, "${
        todo.name
      ...
      if (todo.subtasks) {
        ...
        todo.subtasks.forEach((subtask, subtaskId) => {
          liTag += `<li class="subtask">
                          <label for="${id}-${subtaskId}">
                              <input onclick="updateSubtaskStatus(${id}, ${subtaskId})" type="checkbox" id="${id}-${subtaskId}" ${
            subtask.status === "completed" ? "checked" : ""
                          ...
                          <div class="subtask-settings">
                              <ul class="task-menu">
                                  <button class="inside_btn" onclick='editSubtask(${id}, ${subtaskId}, "${
            subtask.name
          }", "${subtask.deadline}", "${subtask.priority}", "${
            subtask.category
          }")'><i class="uil uil-pen"></i>Edit</button>
...
function makeSubtasksDraggable(taskId) {
  const subtasks = document.querySelectorAll(
    `.task[data-task-id="${taskId}"] .subtask`
  );
